...
//            m_iColumnCount = -1;
        ...
        // XXX: CINEMA
        public Byte getByte(int columnIndex) throws BasicException {
            try {
                byte iValue = m_rs.getByte(columnIndex);
                return m_rs.wasNull() ? null : new Byte(iValue);
            } catch (SQLException eSQL) {
                throw new BasicException(eSQL);
            }
        }
        // XXX: CINEMA
        public Character getCharacter(int columnIndex) throws BasicException {
            try {
                char iValue = m_rs.getString(columnIndex).charAt(0);
                return m_rs.wasNull() ? null : new Character(iValue);
            } catch (SQLException eSQL) {
                throw new BasicException(eSQL);
            }
        }
        ...
        public Short getShort(int columnIndex) throws BasicException {
            try {
                short iValue = m_rs.getShort(columnIndex);
                return m_rs.wasNull() ? null : new Short(iValue);
            } catch (SQLException eSQL) {
                throw new BasicException(eSQL);
            }
        }
